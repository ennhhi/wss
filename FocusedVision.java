public class FocusedVision extends Vision {
    ...
    private Path findClosest(String resource) {
        java.util.List<Direction> directions = java.util.List.of(Direction.EAST, Direction.NORTH_EAST, Direction.SOUTH_EAST);
        for (Direction dir : directions) {
            Tile tile = map.getTileInDirection(dir);
            if (tile != null && hasResource(tile, resource)) {
                return new Path(java.util.List.of(dir),
    ...
    private Path findLowestCost() {
        ...
        for (Direction dir : directions) {
            ...
            if (tile != null) {
                ...
                if (cost < bestCost) {
                    bestCost = cost;
                    best = new Path(java.util.List.of(dir), cost,
